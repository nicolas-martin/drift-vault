 ...
 */
function validateOracleFeed(data) {
    if (!data || typeof data !== 'object') {
        return { success: false, error: 'Expected object' };
    }
    const obj = data;
    if (!index_js_1.NonEmptyArrayUtils.safeValidate(obj.jobs)) {
        return {
            success: false,
    ...
    for (let idx = 0; idx < jobs.length; idx++) {
        try {
            (0, oracle_job_js_1.normalizeOracleJob)(jobs[idx]);
        }
        catch (error) {
...
function normalizeOracleFeed(data) {
    const parseFeedObject = (data) => {
        ...
        if (!validation.success)
            throw new OracleFeedError(validation.error);
