...
/**
 * Validates an OracleJob definition.
 */
function validateOracleJob(data) {
    if (!data || typeof data !== 'object') {
        return { success: false, error: 'Expected object' };
    }
    const obj = data;
    if (!NonEmptyArrayUtils.safeValidate(obj.tasks)) {
        return {
            success: false,
            error: '"tasks" property is expected to be a non-empty array',
        };
    }
...
export function normalizeOracleJob(data) {
    const parseJobObject = (data) => {
        ...
        const validation = validateOracleJob(data);
        if (!validation.success)
            throw new OracleJobError(validation.error);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return OracleJob.fromObject(validation.data);
    };
